...
void setup() {
   ...
   size (1200, 700); //define size of window(width, height)
   
   myPort = new Serial(this,"COM3", 9600);     //starts serial communication with COM3 at 9600baud              
   myPort.bufferUntil('.'); // reads data from serial port until period
  background(0);
}
void serialEvent (Serial myPort) {                                                     // starts reading data from the Serial Port
                                                                                      // reads the data from the Serial Port up to the character '.' and puts it into the String variable "data".
      data = myPort.readStringUntil('.'); // reads serial input until each "."
      data = data.substring(0,data.length()-1); // divides data into substrings
      
      int index1 = data.indexOf(",");   //defines data indexer                                                 
      ang= data.substring(0, index1);   // i or angle is first item on data or index1                                            
      dist = data.substring(index1+1, data.length());  //distance is second item on data or index1+1                           
      
      angle = int(ang); //turns ang into angle integer
      distance = int(dist); //turns distance into dist integer
...
void drawObject() {
    ...
    float pixelDist = (distance/200.0)*(width/2.0);                        // covers the distance from the sensor from cm to pixels
    float pd=(width/2)-pixelDist;
    
             
    float x=-pixelDist*cos(radians(angle));
    float y=-pixelDist*sin(radians(angle));
    
    if(distance<=200)                                                  // limiting the range to 200 cms
    {                               
...
void drawText()
{
    ...
    text("50cm",(width/2)+(width*0.115),height*0.93);
    text("100cm",(width/2)+(width*0.24),height*0.93);
    text("150cm",(width/2)+(width*0.365),height*0.93);
    text("200cm",(width/2)+(width*0.45),height*0.93);
    
    textSize(40);
    text("Project:SONAR",width*0.08,height*0.99);
    text("Angle :"+angle,width*0.45,height*0.99);

    for (int distance = 0; distance <= 200; distance += 200)
     {
      text("Distance :"+distance,width*0.7,height*0.99);
    }
